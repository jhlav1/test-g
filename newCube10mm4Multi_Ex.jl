function generate_gcode_multi_object(filename)
    open(filename, "w") do file
        ...
        # Settings for multiple cubes
        bed_positions = [(100.0, 100.0), (150.0, 100.0), (100.0, 150.0)] # Positions of objects
        ...
        # Generate G-code for each layer, switching between objects
        for layer in 1:num_layers
            z_height = 0.2 + (layer - 1) * layer_height
            println(file, "G1 Z$z_height F300 ; Move to layer $layer")

            for (x_center, y_center) in bed_positions
                println(file, "; Start object at ($x_center, $y_center)")

                # Draw square for the current object
                for i in 1:segment_points
                    t = (i - 1) / (segment_points - 1) # Normalize point distribution
                    x1, y1 = x_center - side_length / 2 + t * side_length, y_center - side_length / 2
                    x2, y2 = x_center + side_length / 2, y_center - side_length / 2 + t * side_length
                    x3, y3 = x_center + side_length / 2 - t * side_length, y_center + side_length / 2
                    x4, y4 = x_center - side_length / 2, y_center + side_length / 2 - t * side_length
...
generate_gcode_multi_object("new_multi_cube_x.gcode")
